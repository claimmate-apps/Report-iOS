...
- (void)viewDidLoad {
    ...
    txtMr.text = [arrMr objectAtIndex:0];
    ...
    dictDweling2 = [[NSDictionary alloc] initWithDictionary:[self getDefault:@"dwl_second"]];
    if (dictDweling2 != nil) {
...
- (void)setupScrollView {
    ...
    viewMainTimeOfInspected.frame = CGRectMake(viewMainTimeOfInspected.frame.origin.x, viewMainDateOfInspected.frame.origin.y, viewMainTimeOfInspected.frame.size.width, viewMainTimeOfInspected.frame.size.height);
...
- (IBAction)onClickInside:(id)sender {
    ...
    NSDictionary *parameters2 = @{
                                 ...
                                 @"misc_aps_damage_custome": isAPSDamage ? [APPDELEGATE isEmpty:txtAPSDamage.text] : @"",
